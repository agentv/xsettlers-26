...
- **Detection is separate from fog of war, deliberately.** `player_sectors` records knowledge of the *ground*, which stays true once learned; `org_sightings` records what a scan saw of other players' *organizations*, which move — so a sighting is a dated observation, never current intel, and every reader presents it with `seen_at_turn`. `_resolve_scan` (`engine/turn.py`) calls `db/sightings.record_sightings()`, which rolls a per-org d6 (`DETECTION_THRESHOLD` of `DETECTION_DIE_SIDES`, currently 6/6 — certain, but still rolled, so lowering the threshold changes odds without shifting a seeded run's roll sequence) and upserts one row per (observer, org). `owner_id`/`org_type` are denormalized on purpose: they are part of what was *observed*, and joining `organizations` would report today's truth about a ship last seen ten turns ago. Detection has its own RNG salted off `SECTOR_ROLL_SEED`, same reason `map_layout_rng()` does. `show_sector_neighborhood` keeps the two sources strictly apart — `"R"` (live, only for sectors you occupy) versus `"r"` (remembered, dated).
