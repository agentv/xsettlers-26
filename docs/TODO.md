...
* [ ] **`show_sector_neighborhood`: shrink default radius 5 → 4, and trim the header.** Queued 2026-08-02 during interactive play-testing. Two changes: `NEIGHBORHOOD_RADIUS` in `xsettlers_mcp/tools/sector_tools.py` drops from 5 to 4 (9×9 grid instead of 11×11); and `display.header` (currently `f"Neighborhood of {origin} — radius {radius}, z-plane {cz}" + (f", turn {current_turn}" ...)`) drops the `radius` and `z-plane` clauses as noise — nothing has ever placed an org off `z=0`, and the radius is a fixed default a player doesn't need repeated back to them. Header becomes just `f"Neighborhood of {origin}"` plus the turn clause. Not yet implemented — queued, not done.
