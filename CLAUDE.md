...
`home_sector_energy` is **calibrated, not arbitrary**: 2200 is the figure at which a fleet that colonizes in place on turn one and never moves empties its home sector on turn 16 of a 20-turn game. Changing it changes when — or whether — a passive player ever feels scarcity. Note the trap before tuning against it: energy scores 0 while `produce_energy` costs food, so a sector dying currently makes the score rate *rise*, and the real stall comes several turns later when stored energy runs out. `docs/TODO.md` tracks this. **The length of the participants list is the scenario's player count**, so a solo game (`config/game_solo.yaml`) and a five-player game differ only in YAML; no code branches on player count. `max_players` is an engine ceiling, never a floor.
