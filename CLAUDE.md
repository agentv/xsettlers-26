...
- **Mission vs task terminology — organizations have a `mission`, pods have a `task`.** One word for each of two different concepts: `organizations.mission` is what the *vehicle* is doing (`idle`/`move`/`colonize`/`defend`/`attack`), `pods.task` is what the *crew* is doing (`idle`/`produce_energy`/`produce_food`/`produce_goods`/`scan`). The tool is `set_pod_task`. Renamed in `e9ecbf0` ("Pods have a task; organizations have a mission") because one word covering both meant a player reading "mission: idle" on a ship report reasonably concluded its pods were idle too. This entry previously asserted the exact opposite — that pods use `mission` and that `task`/`set_pod_task` was "older" vocabulary — which was stale by the time it was written and would have led someone to undo the rename; corrected 2026-08-11. `docs/mcp_server_layer_design.md` still uses the pre-rename vocabulary and is superseded on this point by `docs/product_requirements.md` and `docs/data_model_and_storage_design.md`.
...
- `events` is a write-ahead log: player-action events store deltas, and `turn.snapshot` events store a **per-player ledger row** each turn (holdings, score, derived waste — written by `engine/turn.py`'s `_snapshot_holdings`). Note this is *not* a full-state recovery checkpoint, despite the name — a second, unused writer that did dump full state under the same `turn.snapshot` event type was deleted on 2026-08-11, along with the reader that would have confused the two. There is no replay/disaster-recovery mechanism today. `events.resolve_at_turn` drives deferred resolution (e.g. `colonize_complete` fires 3 turns after `set_mission('colonize', ...)`).
- **There is no `models/` package.** CRUD logic lives directly in `xsettlers_mcp/tools/*.py`. A `models/` directory of six 0-byte files used to sit here advertising a layer that did not exist; it was deleted 2026-08-11 with no code change, since nothing imported it. Pulling CRUD out into a real model layer is still tracked in `docs/TODO.md` — recreate the package then, not before.
