...
- **Scanning is scanning, whoever carries the equipment** — an org's innate sensors and a pod on the `scan` task obey identical rules, implemented once in `engine/scanning.py` and resolved once in `engine/turn.py`'s `_resolve_scan`. Don't reintroduce a pod-specific copy. An aim is an **offset from the scanner's own sector**, not absolute coordinates, so it survives a move (`engine/bearings.py`).
