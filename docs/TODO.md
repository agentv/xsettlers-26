...
* [ ] **The strategies repeat their own opening.** `_fan_out_consolidate`, `_burst_and_colonize` and `_fan_out` each open by running the same two queries — every ship id for the player (`SELECT id FROM organizations WHERE player_id=? AND org_type='ship' ORDER BY id`) and then the home coordinates off the first of them — and each then writes its own "no ships, mark done, bail" guard. Three copies of ~10 lines. Noted in the 2026-08-11 complexity audit and deliberately left: unlike the other duplication found that day, this one is inside code that is still actively changing shape (`_fan_out` was rewritten as recently as 2026-08-10), and consolidating strategies onto a shared preamble before their common shape has settled would freeze the wrong abstraction. Fold it into a helper once the strategy set stops moving — and note `ORDER BY id` is load-bearing, since bootstrap deliberately numbers every player's ships before any colony (see `db/bootstrap.py`).
