...
- **A sector's richness depends on where it is.** `reveal_sector()` rolls 400 + d6×100 and scales it by `richness_multiplier()` — the largest multiplier among the `map_hotspots` rows covering that coordinate, 1.0 in open space. Overlapping regions take the **max, not the product**, so a scenario's stated multipliers are the real ceiling rather than an accident of geometry; the multiplier scales the **whole roll**, so a ×3 region's floor (1500) clears open space's ceiling (1000). The layout is seeded once by `db/bootstrap._seed_map()` from the scenario's `map:` block *before the first reveal*, and read from the DB thereafter — a `scatter:` rule is expanded with the map seed at bootstrap, so the placement exists only once and must survive a restart. **No tool exposes `map_hotspots`**: a player learns a region exists by revealing into it, which is what lets a scenario keep its map secret. `home_sector_energy` is deliberately *not* a hotspot — it's an absolute figure written over whatever home rolled, because a starting position is a promise about a number rather than a bet on a range.
...
- **Scanning reveals occupants, and that intel is per sector and ages with it.** `_resolve_scan` (`engine/turn.py`) calls `db/sightings.record_sightings()`, which rolls a per-org d6 (`DETECTION_THRESHOLD` of `DETECTION_DIE_SIDES`, currently 6/6 — certain, but still rolled, so lowering the threshold changes odds without shifting a seeded run's roll sequence) and writes `org_sightings`. **A look is authoritative for its sector**: prior belief about that sector's occupants is cleared and replaced by what was just detected, so an emptied sector reads empty instead of keeping a ghost. Sightings are only ever read through `player_sectors`, so they inherit that sector's confidence and blink out with it at 0 — there is no second expiry rule. `owner_id`/`org_type` are denormalized on purpose: they are part of what was *observed*, and joining `organizations` would report today's truth about a ship last seen ten turns ago. Detection has its own RNG salted off `SECTOR_ROLL_SEED`, same reason `map_layout_rng()` does. `show_sector_neighborhood` distinguishes `"R"` (live, only for sectors you occupy) from `"r"` (from a scan, carrying `sighted_at_turn`), and suffixes `">"` onto every cell your fleet is scanning this turn (`_scan_coverage`, itemized in `scan_aims`). **Coverage is a question about the frame, not about the centered org**: it counts every scanner of yours whose target lands in the viewport, including one standing outside it, since scan range reaches across the frame edge. Each aim resolves against **its own scanner's position** — an aim is an offset, so resolving against the viewport center is only correct for the centered org and silently wrong for every other. An org in transit contributes nothing (no position, and end-of-turn suppresses its scan anyway). Aims landing off-plane or outside the frame are dropped, not reported — `show_organization` is where a given ship's own bearings are listed. The marker is applied by `_draw_grid` as a layer over the viewport rather than folded into a sector's cell string, because a scan is usually aimed at a sector nobody has seen, which has no row to decorate. `engine/scanning.scanners_on` is the single answer to "what is this org looking at", so the map and `show_organization`'s bearing list cannot disagree.
