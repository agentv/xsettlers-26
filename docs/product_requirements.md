...
* A game instance is defined by a single scenario file (e.g. `config/game2.yaml`) and a corresponding SQLite database — one shared game per deployed instance.
...
* **Home sectors are exempt from the discovery roll.** Each player's starting sector is seeded flat at `home_sector_energy`, a per-scenario setting (`HOME_SECTOR_ENERGY` = 750 is only the fallback for a scenario that doesn't set it; every shipped scenario sets it explicitly, to 2,200 — well above the discovery ceiling of 1,000). A player's own footing should never be what runs out from under them, and that is precisely what allows the frontier to be lean. **This is the home sector, not the sentinel sector** — see below.
