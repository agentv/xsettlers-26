...
* [ ] **Per-scenario lobby sizing, on GameHouse's side.** `register_game` carries one `min_players`/`max_players`/`wait_window_seconds` for the whole game (they live on GameHouse's `game` row; `game_scenario` holds only a `scenario_key`), but a scenario's player count is derived from its own participants list. xsettlers therefore publishes only the scenarios matching the registered shape — today `game0`, `game1`, `game_crowd` at 2 players/120s — and withholds `game_solo`, which is 1 player on a 0s window, so it cannot be offered through GameHouse at all. The fix is moving those three fields onto `game_scenario`; until then `registrable_scenarios()` (`xsettlers_mcp/gamehouse.py`) is what keeps a mis-sized lobby from being offered.
