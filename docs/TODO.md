...
* [ ] Authentication / trust boundary — `xsettlers_mcp/auth.py` trusts `slack_user_id` at face value; harden for production (blocked on the gateway build-out below). Now doubly relevant with a network-reachable HTTP endpoint instead of a locally-spawned stdio process — anything that can reach the Fly.io URL can call `/mcp`, not just a co-located Slackbot process.
