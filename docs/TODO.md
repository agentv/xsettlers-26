...
* [ ] **Tick countdown is computed in two places** — `scripts/status.py`'s `_clock_status` and `organization_reports.py`'s `_tick_countdown_display` both parse `next_tick_at` and `divmod` it into minutes/seconds (~5 lines each). Deliberately *not* merged during the 2026-08-11 complexity audit: they genuinely differ in what they can know. The CLI runs as its own process and can health-check the server to distinguish "paused" from "counting down"; the in-process one cannot, so a `None` there already means "not running". Both files already comment on the relationship. Worth revisiting only if a third caller appears — at which point the shared piece is the formatting, not the liveness judgement.
...
* [x] ~~**The strategies repeat their own opening.**~~ **Resolved 2026-08-11 by deleting the strategies, not by refactoring them.** The three that shared the preamble — `_fan_out_consolidate`, `_burst_and_colonize` and `_turtle` — are now YAML programs under `config/npc_programs/`, and `engine/npc_script.py`'s runner holds the single remaining copy of the ships-lookup (`ORDER BY id` still load-bearing, for the same `db/bootstrap.py` reason). `_fan_out` keeps its own because it is a policy, not a program, and stays code. The note was right to say "fold it into a helper once the strategy set stops moving" — what actually stopped moving was the set of things written *in Python*.

* [ ] **A program cannot size a selection relative to the fleet.** `_burst_and_colonize` computed its colonizing cohort as `round(len(ships) * colonize_fraction)`; `config/npc_programs/burst_and_colonize.yaml` names a fixed `slice: [0, 2]` instead. Identical for the 8-ship fleets every scenario actually uses (`round(8 * 0.25) == 2`), and the tournament scores confirm it, but they diverge at other fleet sizes — a 1-ship fleet now colonizes where the Python fanned out. A fractional/proportional selector (`{fraction: 0.25}` beside `slice`/`stride`/`offset`) is the obvious fix; deferred rather than smuggled in, since nothing in play needs it yet. Covered by a test that asserts the divergence deliberately (`tests/test_npc_script.py`, `tests/test_npc.py`) so it cannot become a silent surprise.

* [ ] **`_fan_out_consolidate`'s 8-ship floor is gone, not ported.** The Python refused to act at all below 8 ships (4 directions × 2); a slice just selects fewer. Nothing depends on the old behavior, and no scenario runs a short fleet — but if a minimum-fleet precondition is ever actually wanted, it belongs in the program format as a guard, not rediscovered per strategy.
...
All four are now real and registered (not just named/characterized) — live-verified via a real NPC seated through the GameHouse handoff above, running `fan_out` under `end_of_turn()` and actually reacting to scan results.

**Since 2026-08-11 a strategy is either data or code, and the split is the taxonomy's own line drawn in the implementation.** `turtle`, `burst_and_colonize` and `fan_out_consolidate` are *plans* — fixed openings whose whole sequence is decided in advance — and now live as `config/npc_programs/*.yaml`, executed by `engine/npc_script.py`. `fan_out` and `frontier_map_stay_frosty` are *policies* — they read the world each turn and decide from it — and stay as functions in `engine/npc.py`. Adding a scripted strategy is adding a YAML file, the same property scenarios already have. `engine/npc.strategy_names()` is the union of both, so a strategy crossing that line does not change what GameHouse will accept in a roster or what the tournament plays. See "NPC behavior as data" in `docs/dev_history.md`.
...
The point of moving strategies into data (2026-08-11) is a later phase where a strategy is *authored* rather than written. What exists is the format and its validator (`engine/npc_script.validate_program`, run at assign time by `assign_npc_profile()` so an authoring tool gets its errors synchronously). Outstanding before a builder is worth writing:

* [ ] **No way to list or describe the vocabulary programmatically.** A builder needs the legal actions, phases and selector keys as data it can render into a form. They are currently constants (`ACTIONS`, `QUEUED_PHASES`, `STEP_KEYS`) plus prose in the module docstring.
* [ ] **Programs are single-phase by construction.** `run_program` dispatches once and sets `memory["dispatched"]`. That is exactly right for an opening and exactly wrong for anything with a second act that isn't expressible as a ship's-log trigger. Whether the format grows phases, or reactive strategies simply stay code, is the open design question — don't answer it by accident.
* [ ] **`assign_npc_profile()` is still dev/test-only**, so an authored program has no path into a live game except through the GameHouse handoff's roster. A builder needs one.
* [ ] Fleet-relative selectors (see the `burst_and_colonize` divergence above) and a fleet-minimum guard.
